...
.participantes-table th { background:#f5f5f5; padding:10px 12px; text-align:left; border-bottom:2px solid #ddd; white-space:nowrap; font-size:13px; color:#555; }
...
.tag-lugar{ display:inline-block; background:#fef9e7; color:#b7770d; border-radius:4px; padding:2px 8px; font-size:12px; }

/* ===== BOTONES FILA ===== */
.btn-sm   { padding:5px 10px; border-radius:5px; border:none; cursor:pointer; font-size:12px; font-weight:500; }
...
/* ===== CARDS MOBILE (reemplaza tabla en mobile) ===== */
@media (max-width: 700px) {
    .participantes-table thead { display:none; }
    .participantes-table, .participantes-table tbody, .participantes-table tr, .participantes-table td { display:block; width:100%; }
    .participantes-table tr { background:#fff; border:1px solid #e8e8e8; border-radius:8px; margin-bottom:10px; padding:12px 14px; }
    .participantes-table tr:hover td { background:transparent; }
    .participantes-table td { padding:4px 0; border:none; }
    .participantes-table td:last-child { padding-top:10px; display:flex; gap:8px; }
    .td-nombre { font-size:15px; font-weight:600; color:#222; }
    .td-meta   { display:flex; flex-wrap:wrap; gap:6px; margin:6px 0 4px; }
    .td-tel    { font-size:13px; color:#666; }
    .td-tel::before { content: "📞 "; }
    .td-lugar-wrap::before { content: "📍 "; }
}
...
.modal-content { background:#fff; border-radius:10px; padding:28px; width:100%; max-width:480px; max-height:90vh; overflow-y:auto; }
...
.modal-content input,
.modal-content select { width:100%; padding:8px 10px; border:1px solid #ddd; border-radius:6px; font-size:14px; box-sizing:border-box; }
.modal-content input:focus,
.modal-content select:focus { outline:none; border-color:#FF6600; }
...
<div class="part-toolbar">
    <div class="part-search">
        <input type="text" id="inputBuscar" placeholder="Buscar por nombre, apellido, DNI o localidad..." autocomplete="off">
    </div>
        ...
        <form id="formParticipante" novalidate>
            ...
            <div class="modal-grid">
                ...
                <div>
                    ...
                    <input type="text" id="fLugar" placeholder="Ej: Los Telares">
                </div>
                <div>
...
document.getElementById('inputBuscar').addEventListener('input', function() {
    ...
    if (q === '') { renderTabla(todosLosParticipantes); return; }
...
function renderTabla(lista) {
    ...
    if (!lista || lista.length === 0) {
        tbody.innerHTML = '<tr><td colspan="6" class="no-results">No se encontraron participantes.</td></tr>';
        count.textContent = '';
        return;
    }
    ...
    tbody.innerHTML = lista.map(p => `
        <tr id="row-${p.id}">
            <td class="td-nombre"><strong>${esc(p.apellido)}, ${esc(p.nombre)}</strong></td>
            ...
            <td class="td-lugar-wrap">${p.lugar ? `<span class="tag-lugar">${esc(p.lugar)}</span>` : '<span style="color:#ccc">—</span>'}</td>
            <td class="td-tel" style="color:#666">${esc(p.telefono || '—')}</td>
            <td style="white-space:nowrap">
                <button class="btn-sm btn-edit" onclick="abrirModalEditar(${p.id},'${esc2(p.nombre)}','${esc2(p.apellido)}','${esc2(p.dni||'')}','${esc2(p.telefono||'')}','${esc2(p.categoria||'')}','${esc2(p.lugar||'')}')">Editar</button>
...
function abrirModalEditar(id, nombre, apellido, dni, telefono, categoria, lugar) {
    ...
    document.getElementById('fLugar').value     = lugar;
    document.getElementById('modalForm').classList.add('open');
    document.getElementById('fNombre').focus();
}
...
function limpiarForm() {
    ['fNombre','fApellido','fDni','fTelefono','fCategoria','fLugar'].forEach(id => {
...
document.getElementById('formParticipante').addEventListener('submit', function(e) {
    ...
    if (!nombre || !apellido) { showToast('Nombre y Apellido son obligatorios.', true); return; }
    ...
    formData.append('lugar',     document.getElementById('fLugar').value.trim());

    const btn = document.getElementById('btnGuardar');
    btn.disabled = true;
    btn.textContent = 'Guardando...';

    fetch(id ? URL_ACTUALIZAR + id : URL_GUARDAR, { method: 'POST', body: formData })
        .then(r => r.json())
        .then(resp => {
            if (!resp.ok) { showToast('Error al guardar.', true); return; }
            cerrarModal();
...
function eliminar(id, nombre) {
    ...
    fetch(URL_ELIMINAR + id, { method: 'POST' })
        .then(r => r.json())
        .then(resp => {
            if (!resp.ok) { showToast(resp.error || 'No se pudo eliminar.', true); return; }
            const row = document.getElementById('row-' + id);
            if (row) row.remove();
            todosLosParticipantes = todosLosParticipantes.filter(p => p.id != id);
            showToast('Participante eliminado.');
            if (!document.querySelector('#tbodyParticipantes tr[id]')) renderTabla([]);
            actualizarCount();
        })
        .catch(() => showToast('Error de conexión.', true));
}
...
function recargarLista() {
    ...
    fetch(q ? URL_LISTAR + '?q=' + encodeURIComponent(q) : URL_LISTAR)
        .then(r => r.json())
        .then(data => {
            if (!q) todosLosParticipantes = data;
            renderTabla(data);
        });
}
